...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        dataset = events.metadata["dataset"].split('_')[0] #cambio
        # Get the gen leptons
        ...
        # Leptons
        lep = lep[(lep.pt>25) & (np.abs(lep.eta) < 2.4)]
        nleps = ak.num(lep)
        
        # Jets
        jet = jet[(jet.pt>30) & (np.abs(jet.eta) < 2.4)]
        njets = ak.num(jet)
        #isBtagJetsMedium = isbjet(jet.btagDeepFlavB,year)
        #isNotBtagJetsMedium = np.invert(isBtagJetsMedium)
        #nbtagsm = ak.num(jet[isBtagJetsMedium])   
        # Met cut (30 GeV)
        metcut = (met.pt >= 30)
