from config import *
#he puesto todos los fondos de ttbar
#prdic = {'data':'MuonEG,EGamma,DoubleMuon,SingleMuon,DoubleEG','other': 'TTTo2L2Nu, tbarW, tW, WJetsToLNu, TTToSemiLeptonic, WW, WZ, ZZ',   'DY': 'DYJetsToLL_M50, DYJetsToLL_M10to50'}
prdic = {'data':'MuonEG,DoubleMuon,SingleMuon,SingleElectron,DoubleEG,EGamma','other': 'TTTo2L2Nu, tbarW_noFullHad, tW_noFullHad, WJetsToLNu_MLM, TTToSemiLeptonic, WWTo2L2Nu, WZ,ZZ, ZZZ_ext1, WWW_ext1, WWZ_ext1, WZZ_ext1TTZToQQ,TTZToLL_M_1to10, TTZToLLNuNu_M_10,TTWJetsToLNu,TTWJetsToQQ, TTG',   'DY': 'DYJetsToLL_M_10to50_MLM, DYJetsToLL_M_50_MLM'}
...
def DrawDY(var='invmass', chan=ch, level=level, doLog=True):
  ...
  hem = GetDYhist(plt, 'em', level, None, var, integrateAxis=False) #'data'
  kll, kllerr = res['kll'][chan]
  hem.scale(kll*0.5)
  
  hem = hem.group("process", hist.Cat("process", "process"), {datname:'data'})

  p = plotter(path, {'Drell-Yan': 'DYJetsToLL_M_10to50_MLM, DYJetsToLL_M_50_MLM'}, bkgList=['Drell-Yan'], colors={'Drell-Yan':'#3b78cb', datname:'#c6c3b0'}, lumi=lumi, var=var)
  p.SetDataName('data')
  if doLog: p.SetLogY()
  p.plotData = True
  p.SetRatio(True)
...
def DYDD(plt, level='dilep', save=False):
  ...
  if save:
    t = OutText(outpath, 'DYDD_'+level, 'new', 'tex')
    t.bar()
